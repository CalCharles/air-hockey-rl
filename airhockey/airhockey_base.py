...
class AirHockeyBaseEnv(ABC, Env):
    # Tasks whose paddle starts at a uniformly random reachable pose rather than
    # the fixed 3/4-down-the-table pose set this to True. Overridable per config
    # with ``random_paddle_spawn``.
    random_paddle_spawn_default = False
    ...
    def __init__(self, **kwargs):
        ...
        self.defaults = {
            ...
            # Paddle spawn: None defers to the task's `random_paddle_spawn_default`.
            'random_paddle_spawn': None,
            # Puck spawn: uniform over the top `puck_spawn_top_fraction` of the
            # table, speed uniform in [min, max] at a uniformly random heading.
            'puck_spawn_top_fraction': 2.0 / 3.0,
            'puck_spawn_speed_min': 0.0,
            'puck_spawn_speed_max': 0.5,
        }
        ...
        self.random_paddle_spawn = (
            self.random_paddle_spawn_default
            if config.random_paddle_spawn is None
            else bool(config.random_paddle_spawn)
        )
        self.puck_spawn_top_fraction = float(
            np.clip(config.puck_spawn_top_fraction, 0.0, 1.0)
        )
        self.puck_spawn_speed_min = float(max(0.0, config.puck_spawn_speed_min))
        self.puck_spawn_speed_max = float(max(0.0, config.puck_spawn_speed_max))
        if self.puck_spawn_speed_max < self.puck_spawn_speed_min:
            raise ValueError("puck_spawn_speed_max must be >= puck_spawn_speed_min")
        self.simulator_params = simulator_params
    ...
    def sample_paddle_spawn_in_workspace(self, margin=0.0):
        ...
        _, _, y_low, y_high = self.get_paddle_workspace_bounds(margin=margin)
        y_pos = float(self.rng.uniform(low=y_low, high=y_high))
        x_low, x_high, _, _ = self.get_paddle_workspace_bounds(margin=margin, y=y_pos)
        x_pos = float(self.rng.uniform(low=x_low, high=x_high))
        return (x_pos, y_pos), (0.0, 0.0)

    def sample_puck_spawn_top_fraction(self, bad_regions=None):
        """Uniform puck spawn over the top ``puck_spawn_top_fraction`` of the table.

        Speed is uniform in ``[puck_spawn_speed_min, puck_spawn_speed_max]`` at a
        uniformly random heading, so no direction is privileged.
        """
        x_low = self.table_x_top + self.puck_radius
        x_high = self.table_x_top + self.puck_spawn_top_fraction * self.length
        x_high = float(np.clip(x_high - self.puck_radius, x_low, self.table_x_bot - self.puck_radius))
    ...
    def get_paddle_configuration(self, name):
        ...
        # Default behavior
        if name == 'paddle_ego':
            if self.random_paddle_spawn:
                return self.sample_paddle_spawn_in_workspace()
