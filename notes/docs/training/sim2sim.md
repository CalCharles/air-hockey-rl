...
> **Canonical big-gap target as of 2026-05-07**: `configs/new_juggle/sim2sim_warp075_p30.yaml` — paddle −30% (mass-preserved) + sine-y puck-obs warp 0.075, all delays / hist_len / restitutions matched to source. zs ≈ 48; from-scratch peak 112 at 400k. Replaces the previous `sim2sim_combined.yaml` (paddle50). See [`notes/scratch/experiments/2026-05-07_15-48_sim2sim-target-redesign.md`](../../scratch/experiments/2026-05-07_15-48_sim2sim-target-redesign.md) for the redesign rationale.
>
> The previous canonical big-gap target — `sim2sim_combined.yaml` — was structurally untrainable from-scratch (peak 84, mean 47 at 3.85M). That made the "improvement over zs" claim for any residual recipe on it unfalsifiable. Separately, every v25–v30 paddle50 recipe (`v27_ensemble5`, `v30_explore_lite`, …) was trained with a silent Polyak-averaging bug discovered 2026-05-06. Recipe development on the new target is in flight; until results land, **do NOT use the v27 or v30_explore_lite paddle50 configs as recipe-level guidance** — they're historical-only.
>
> For the small-gap recipe (<10% zs drop) and for residual-RL infrastructure docs, [`training/residual-rl-recipe.md`](residual-rl-recipe.md) remains the canonical entry point.
...
- **Source sim**: any `configs/new_juggle/sysid_best_params*.yaml`. The canonical source is now `sysid_best_params_hist2.yaml` (matches `latest_models/canonical/hist2_motion0/config.yaml`). The hist3 / hist4 / hist5 variants used for the original temporal-smoothing ablation are preserved at `configs/new_juggle/legacy/sysid_best_params_hist{3,4,5}.yaml`. The reference campaign below is the one that was actually run against `sysid_best_params_hist4.yaml` before the legacy move; new sim2sim work should source from `sysid_best_params_hist2.yaml`.
- **Target sim**: lives next to source as `configs/new_juggle/sim2sim_<tag>.yaml`. Inherits structurally from one source — only physics keys differ. First line is `# Source: <source_yaml>` for provenance. Each modified key has an inline `# PERTURBED: ...` comment.
  - **Active targets**:
    - **`sim2sim_warp075_p30.yaml` — current canonical big-gap target (2026-05-07)**. Paddle −30% (mass-preserved) + edge-preserving sine y-warp 0.075 on the puck observation. All delays / hist_len / restitutions / wall_cone / pid_kp held at source values. zs ≈ 48; from-scratch peak 112 at 400k. Designed so adaptation success is a credible proxy for sim2real (multi-modal mismatch: geometric + perceptual) AND from-scratch is achievable so improvements are falsifiable. See [`notes/scratch/experiments/2026-05-07_15-48_sim2sim-target-redesign.md`](../../scratch/experiments/2026-05-07_15-48_sim2sim-target-redesign.md). Sibling targets `sim2sim_warp075_p{00,10,20}.yaml` provide gentler-paddle alternatives (zs={58, 49, 60}) on the same warp axis.
    - `sim2sim_combined.yaml` (paddle50, **deprecated 2026-05-07**) — paddle −50% (mass-preserved) + dynamics deltas (`pid_kp` −20%, action delay on, `wall_direction_cone_deg` 10→25). zs ≈ 67.5 but **from-scratch can't train** (3.85M plateau at mean 47, peak 84). Kept on disk for reproducing past v25–v30 / paddle50 experiments only; do NOT use for new sim2sim work.
    - `sim2sim_combined_warp.yaml` — `sim2sim_combined.yaml` + a third perturbation (`puck_obs_sine_warp_amplitude: 0.05`). Same structural untrainability issue inherited from `sim2sim_combined.yaml` — superseded by `sim2sim_warp075_p30.yaml`.
