...
> **Note on warm-start path layout.** `async_td3_real.py` now writes per-episode artifacts under `<data_root_dir>/<model_path_parent_dir>/data_<YYYYMMDD-HHMMSS>/{episode_hdf5,reset_hdf5,episode_gifs,episode_camera_videos}/` (see [`td3-async-replay.md`](../environments/real-world/td3-async-replay.md#launch-commands)). The flat `real_runs/online_run/reset_hdf5` path above refers to a previously collected dataset; for fresh runs you'll want to point `warm_start_hdf5_dirs` at the new nested location (the loader is recursive when `warm_start_hdf5_recursive: true`, so pointing at `real_runs/online_run/` and letting it walk works).
