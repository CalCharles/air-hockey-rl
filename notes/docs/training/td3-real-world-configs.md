...
Used by `extras/async_td3_real_reset_policy.py` — a separate policy trained to reset the puck to a valid state. Structurally simpler: single-stream TD3 (no PER, no success/failure buffer split), `buffer_size: 300000`, `q_updates: 10`, `actor_updates_per_iteration: 2`, `target_network_frequency: 2`. Loads reset episodes from `real_runs/online_run/reset_hdf5`. Adds reset-specific knobs: `max_reset_window_steps: 120`, margin/failure-count thresholds for detecting a bad reset. No motion reward or exploration primitives.
