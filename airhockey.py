...
def set_box2d_as_base_env():
    from Box2D.b2 import world
    from Box2D import (b2CircleShape, b2FixtureDef, b2LoopShape, b2PolygonShape,
           b2_dynamicBody, b2_staticBody, b2Filter, b2Vec2)
    from air_hockey_simulator.airhockey_box2d import AirHockey2D as AirHockeyBaseEnv
    
def set_robosuite_as_base_env():
    from air_hockey_challenge_robosuite.robosuite.wrappers.gym_wrapper import AirHockeyRobosuite as AirHockeyBaseEnv
...
class AirHockey(Env):
    def __init__(self, num_paddles, num_pucks, num_blocks, num_obstacles, num_targets, 
                 ...
                 goal_max_x_velocity, goal_min_y_velocity, goal_max_y_velocity, simulator='box2d',
                 render_masks=False, max_timesteps=1000,  gravity=-5, time_frequency=20):
        ...
        if simulator == 'box2d':
            ...
            from Box2D import (b2CircleShape, b2FixtureDef, b2LoopShape, b2PolygonShape,
                   b2_dynamicBody, b2_staticBody, b2Filter, b2Vec2)
            from airhockey_box2d import AirHockey2D as AirHockeyBaseEnv
            
        elif simulator == 'robosuite':
            from air_hockey_challenge_robosuite.robosuite.wrappers.gym_wrapper import AirHockeyRobosuite as AirHockeyBaseEnv
        else:
            raise ValueError("Invalid simulator. It should be box2d or robosuite.")
        self.simulator_env = AirHockeyBaseEnv(num_paddles, num_pucks, num_blocks, num_obstacles, num_targets,
                                                absorb_target, use_cue, length, width,
                                                puck_radius, paddle_radius, reward_type, max_force_timestep, n_training_steps,
                                                force_scaling, paddle_damping, render_size, wall_bumping_rew,
                                                terminate_on_out_of_bounds, terminate_on_enemy_goal, truncate_rew,
                                                goal_max_x_velocity, goal_min_y_velocity, goal_max_y_velocity, simulator,
                                                render_masks, max_timesteps, gravity, time_frequency)

        # box2d params physics
    ...
    def initialize_spaces(self):
        ...
        if not self.goal_conditioned:
            ...
            if self.reward_type == 'goal_position':
                # y, x
                goal_low = np.array([0, -self.width/2])
                goal_high = np.array([self.length/2, self.width/2])
