...
class HistoryBuffer:
    """
    Circular buffer of observation context for RMA adaptation (φ).

    Maintains the last `context_len` steps so that at any point the
    training loop can call `sample()` to get tensors for the adaptation
    module. Default entries are 6-d paddle/puck pos+valid (no actions).

    Parameters
    ----------
    context_len : history length T fed to the adaptation module
    device      : torch device for snapshot tensors
    include_action : kept for API compatibility; RMA training leaves this
                     False (TD3 last-action-in-policy-state is not used)
    action_dim  : only used when include_action is True
    """
