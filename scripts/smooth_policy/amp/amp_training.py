...
from scripts.smooth_policy.amp.replay_buffer import ReplayBuffer
...
if __name__ == "__main__":
    ...
    # Initialize AMP components if enabled
    if args.amp_enabled:
        print("\n" + "="*80)
        ...
        replay_buffer = None
        demo_loader = None
        print("\n⚠ AMP disabled (amp_enabled=False)\n")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # AMP: Compute discriminator rewards and add to task rewards
        if args.amp_enabled:
            ...
            with torch.no_grad():
                disc_logits = discriminator(norm_disc_obs).squeeze(-1)
                prob = torch.sigmoid(disc_logits)  # P(expert)
                # Reward formula: -log(1 - P(expert)) scaled
                # Higher reward when agent looks more like expert
                disc_r = -torch.log(torch.maximum(1 - prob, torch.tensor(0.0001, device=args.device)))
                disc_r = disc_r * args.disc_reward_scale
            ...
            # Recompute advantages with combined rewards
            with torch.no_grad():
                advantages_combined = torch.zeros_like(combined_rewards).to(args.device)
                lastgaelam = 0
                for t in reversed(range(args.num_steps)):
                    if t == args.num_steps - 1:
                        nextnonterminal = 1.0 - next_done
                    ...
                    advantages_combined[t] = lastgaelam = delta + args.gamma * args.gae_lambda * nextnonterminal * lastgaelam
                returns_combined = advantages_combined + values
            
            # Update advantages and returns with combined version
            b_advantages = advantages_combined.reshape(-1)
            b_returns = returns_combined.reshape(-1)
        ...
        if args.amp_enabled:
            ...
            # Demo = 1 (expert), Agent = 0 (fake)
            bce = nn.BCEWithLogitsLoss()
            disc_loss_demo = bce(disc_demo_logit, torch.ones_like(disc_demo_logit))
            disc_loss_agent = bce(disc_agent_logit, torch.zeros_like(disc_agent_logit))
            disc_loss = 0.5 * (disc_loss_demo + disc_loss_agent)
            ...
            with torch.no_grad():
                disc_agent_acc = (disc_agent_logit < 0).float().mean().item()
                disc_demo_acc = (disc_demo_logit > 0).float().mean().item()
    ...
    # Save AMP components if enabled
    if args.amp_enabled:
