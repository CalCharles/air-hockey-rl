#!/usr/bin/env python3
"""
Prepare AMP (Adversarial Motion Priors) triplet dataset from real robot trajectories.

This script processes all trajectory HDF5 files from /nfs/data/airhockey and creates
a PyTorch tensor dataset consisting of consecutive triplets of states.
...
Output format: PyTorch tensor of shape [N, 3, 4] where:
    ...
    - 3 = state_t, state_{t+1}, state_{t+2}
...
def filter_trajectory(train_vals, min_length=50, safety_check=True):
    ...
    Args:
        ...
        min_length: Minimum trajectory length (must be at least 3 for triplets)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Prepare AMP triplet dataset from real robot trajectories',
    ...
    parser.add_argument(
        '--output-path',
        type=str,
        default='amp_triplet_dataset.pt',
