...
2. Rotational normalization: First velocity rotated to point in (1, 0) direction
3. Keeps only the second state (the first state is now always at origin with normalized velocity)
...
Output format: PyTorch tensor of shape [N, 6]
    ...
    - 6 = [vel1_x, vel1_y, relative_x_pos, relative_y_pos, relative_x_vel, relative_y_vel]
         (vel1 after rotation will be [magnitude, 0])
"""
...
def compute_rotation_matrix(velocity):
    """
    ...
    if magnitude < 1e-8:
        # If velocity is near zero, return identity matrix
    ...
    # This is equivalent to rotating by -atan2(vy_norm, vx_norm)
    # R = [[cos(θ), sin(θ)], [-sin(θ), cos(θ)]] where we want to rotate by -angle
    # Since (vx_norm, vy_norm) = (cos(angle), sin(angle)), we get:
...
def normalize_state_pair(state_pair):
    """
    Normalize a state pair to relative coordinates.
    
    Process:
    1. Translate both states so first position is at (0, 0)
    2. Rotate both states so first velocity points in (1, 0) direction
    3. Return first velocity and second state (position is [0,0] so not kept)
    ...
    pos1_translated = pos1 - pos1  # Now [0, 0]
    pos2_translated = pos2 - pos1
    
    # Step 2: Compute rotation matrix to align first velocity with (1, 0)
    rotation_matrix = compute_rotation_matrix(vel1)
    
    # Apply rotation to positions and velocities
    pos1_rotated = rotation_matrix @ pos1_translated  # Still [0, 0]
    pos2_rotated = rotation_matrix @ pos2_translated
    vel1_rotated = rotation_matrix @ vel1  # Now points in [magnitude, 0] direction
    vel2_rotated = rotation_matrix @ vel2
    
    # Step 3: Return first velocity (contains magnitude info) and second state
    # First position [0, 0] contains no information so not included
    normalized_state = np.concatenate([vel1_rotated, pos2_rotated, vel2_rotated])
...
def validate_normalization(normalized_states, original_state_pairs, num_samples=5):
    ...
    for i in range(min(num_samples, len(normalized_states))):
        ...
        # Verify first velocity is aligned with x-axis (y component should be ~0)
        vel1_y = norm_state[1]
        print(f"  First velocity alignment: y-component = {vel1_y:.6f} (should be ~0)")
...
def save_normalized_dataset(normalized_states, output_path, original_stats=None):
    ...
    save_dict = {
        ...
        'description': 'Normalized AMP dataset with relative state representations',
        ...
        'normalization': {
            ...
            'rotation': 'First velocity rotated to (1, 0) direction',
            'scale': 'Preserved (no scaling applied)',
            'kept': 'First velocity (magnitude info) and second state'
        }
    }
...
def main():
    ...
    print("  2. Rotation: Aligning first velocity with (1, 0) direction")
    print("  3. Extraction: Keeping only second state (first is now normalized)")
    ...
    print("  # vel1 is rotated to (magnitude, 0), second state is relative to first")
