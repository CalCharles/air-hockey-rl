"""
Discriminator network for AMP (Adversarial Motion Priors) with triplet states.

The discriminator learns to distinguish between expert demonstrations and
agent-generated trajectories based on state transition triplets.
"""
...
class Discriminator(nn.Module):
    """
    Discriminator network for AMP with triplet states.
    
    Takes concatenated state triplets as input and outputs a logit
    indicating whether the transition is from expert demonstrations (positive)
    or agent-generated (negative).
    
    Default input dimension is 10 for triplet states with translation-only normalization:
    [vel1_x, vel1_y, rel_pos2_x, rel_pos2_y, rel_vel2_x, rel_vel2_y,
     rel_pos3_x, rel_pos3_y, rel_vel3_x, rel_vel3_y]
    """
    
    def __init__(self, obs_dim=10, hidden_dims=[128, 128], activation='relu'):
        """
        Initialize discriminator.
        
        Args:
            obs_dim: Dimension of discriminator observations (default: 10 for triplet states)
