...
- Expected format: `[N, 2, 4]` state pairs (automatically normalized to `[N, 6]`)
...
The discriminator operates on **normalized consecutive state pairs**:

```
disc_obs = normalized([s_t, s_{t+1}])
         = [vel1_x, vel1_y, relative_x_pos, relative_y_pos, relative_x_vel, relative_y_vel]
```

Where normalization applies **translation only**:
- First position moved to (0, 0) - not included in output
- Velocities preserve original direction (no rotation applied)
- Second position is relative to first
- Format: `[vel1_x, vel1_y, pos2_x-pos1_x, pos2_y-pos1_y, vel2_x, vel2_y]`

This captures the motion dynamics in a translation-invariant manner while preserving velocity directions.
