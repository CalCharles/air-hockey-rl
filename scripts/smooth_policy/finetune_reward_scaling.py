...
class FinetuneRewardScalingArgs:
    ...
    target_base_reward_scaling: float = 0.1  # Target minimum scaling (1/3)
    reward_scaling_factor: float = 0.85  # Factor to multiply by (8.5/10)
...
def test_agent_no_scaling(agent, config, device, num_timesteps=5000):
    """
    Test the agent on an environment with no base reward scaling (scaling=1.0) 
    for a specified number of timesteps and return episode statistics.
    """
    # Create a single test environment with no reward scaling
    test_config = config.copy()
    test_config["base_reward_scaling"] = 1.0
    test_env = AirHockeyEnv(test_config)
    ...
    with torch.no_grad():
        while timesteps < num_timesteps:
            ...
            # Step environment
            next_obs, reward, terminated, truncated, info = test_env.step(action_np)
    ...
    if episodic_returns:
        ...
        # If no episodes completed, use the current partial return
        avg_return = min_return = max_return = current_return
    
    return avg_return, min_return, max_return, len(episodic_returns)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if velocity_magnitudes:
            ...
            if should_decrease_scaling:
                ...
                if args.use_target_base_reward_scaling and not scaling_manager.should_continue():
        ...
        # Test agent on environment with no base reward scaling every 5 iterations
        if iteration % 5 == 0:
            print(f"Testing agent on no-scaling environment at iteration {iteration}...")
            test_avg, test_min, test_max, test_episodes = test_agent_no_scaling(
                agent, config["air_hockey"], args.device, num_timesteps=5000
            )
            
            print(f"No-scaling test results - Avg: {test_avg:.2f}, Min: {test_min:.2f}, Max: {test_max:.2f}, Episodes: {test_episodes}")
            
            # Log to tensorboard
            writer.add_scalar("test_no_scaling/avg_episodic_return", test_avg, iteration)
            writer.add_scalar("test_no_scaling/min_episodic_return", test_min, iteration)
            writer.add_scalar("test_no_scaling/max_episodic_return", test_max, iteration)
            writer.add_scalar("test_no_scaling/num_episodes", test_episodes, iteration)
    ...
    save_tensorboard_plots(log_parent_dir, config, 
        metrics=['charts/avg_episodic_return', 
                ...
                'test_no_scaling/avg_episodic_return',
                'test_no_scaling/min_episodic_return',
                'test_no_scaling/max_episodic_return'])
