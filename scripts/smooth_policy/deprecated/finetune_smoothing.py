...
class FinetuneArgs:
    ...
    performance_threshold: float = 1500.0  # Minimum average performance to maintain
    performance_buffer_iterations: int = 5  # Iterations to check performance before increasing caps
    caps_increment: float = 0.001  # Amount to increase caps coefficients
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if iteration % 5 == 0:  # Log every 5000 steps instead of every 5 steps
            with torch.no_grad():
                ...
                # Calculate total CAPS loss and action regularization
                ...
                writer.add_scalar("eval/caps_loss", caps_loss.item(), global_step)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # CAPS loss (smoothness regularization)
                curr_actions_mean = agent.get_action_mean(b_obs[mb_inds])
                nearby_actions_mean = agent.get_action_mean(nearby_obs[mb_inds])
                
                # Ensure indices do not go out of bounds for next actions
                next_inds = mb_inds + 1
                next_inds = np.clip(next_inds, 0, len(b_obs) - 1)
                next_actions_mean = agent.get_action_mean(b_obs[next_inds])
                
                # Calculate CAPS losses
                ...
                loss = (pg_loss - args.ent_coef * entropy_loss + v_loss * args.vf_coef + 
                       args.action_coef * action_loss + caps_loss)
        ...
        if iteration % 20 == 0:
            ...
            model_path = f"{checkpoint_dir}/finetune_smoothing_model.pth"
    ...
    # Save final model
    torch.save(agent.state_dict(), f"{log_parent_dir}/finetune_smoothing_model.pth")

    # Final evaluation
    evaluate_iterative_smoothing(f"{log_parent_dir}/finetune_smoothing_model.pth", log_parent_dir, config["air_hockey"])
