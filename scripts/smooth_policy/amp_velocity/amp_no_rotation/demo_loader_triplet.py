"""
Demo data loader for AMP training with 3 consecutive states (triplets).

Loads expert demonstration data prepared by prepare_triplet_dataset.py and provides
random sampling for discriminator training.

Supports triplet format: [N, 3, 4] where each triplet contains 3 consecutive states.
"""
...
def normalize_state_triplet_batch(state_triplets):
    """
    Normalize batched state triplets to relative coordinates (translation only).
    
    Process:
    1. Translate all three states so first position is at (0, 0)
    2. Return first velocity and all three states' relative positions and velocities
    ...
    # Step 2: Concatenate first velocity and all subsequent states
    ...
    normalized_state = torch.cat([
        vel1,              # [batch, 2]
        pos2_translated,   # [batch, 2]
        vel2,              # [batch, 2]
        pos3_translated,   # [batch, 2]
        vel3               # [batch, 2]
    ], dim=-1)  # [batch, 10]
...
class DemoLoaderTriplet:
    """
    Loader for expert demonstration data with 3 consecutive states.
    
    Loads state triplets from .pt files created by prepare_triplet_dataset.py and
    provides random sampling for training the discriminator.
    """
    
    def __init__(self, demo_path, device='cuda'):
        """
        Initialize demo loader for triplets.
        ...
        # Load data
        print(f"Loading demonstration triplet data from: {demo_path}")
        data = torch.load(demo_path, map_location=device)
        
        # Auto-detect dataset format
        if 'normalized_triplets' in data:
            # Pre-normalized dataset [N, 10]
            print("  Detected pre-normalized triplet dataset")
            self.demo_obs = data['normalized_triplets'].to(device=device, dtype=torch.float32)
            ...
            print("  Detected non-normalized triplet dataset - applying normalization")
            ...
            print("  Normalizing state triplets...")
            self.demo_obs = normalize_state_triplet_batch(self.state_triplets)
            print(f"  ✓ Normalization complete")
        
        else:
            raise ValueError("Dataset must contain either 'normalized_triplets' or 'state_triplets' key")
        ...
        print(f"✓ Loaded {self.num_demos:,} demonstration triplet observations")
    ...
    def _print_statistics(self):
        ...
        print(f"\n  Demonstration triplet data statistics:")
        ...
        dim_names = [
            'First X Velocity',
            'First Y Velocity', 
            'Second Relative X Position',
            'Second Relative Y Position',
            'Second X Velocity',
            'Second Y Velocity',
            'Third Relative X Position',
            'Third Relative Y Position',
            'Third X Velocity',
            'Third Y Velocity'
        ]
        ...
        vel2_mag = torch.sqrt(self.demo_obs[:, 4]**2 + self.demo_obs[:, 5]**2).mean()
        vel3_mag = torch.sqrt(self.demo_obs[:, 8]**2 + self.demo_obs[:, 9]**2).mean()
        print(f"\n  Normalization check:")
        print(f"    First velocity magnitude (average): {vel1_mag:.6f}")
        print(f"    Second velocity magnitude (average): {vel2_mag:.6f}")
        print(f"    Third velocity magnitude (average): {vel3_mag:.6f}")
