#!/usr/bin/env python3
"""
Prepare AMP dataset with 3 consecutive states (triplets) from real robot trajectories.

This script processes trajectory HDF5 files and creates a PyTorch tensor dataset 
consisting of triplets of consecutive states.

Each state is a 4D vector: [x_position, y_position, x_velocity, y_velocity]

Output format: PyTorch tensor of shape [N, 3, 4] where:
    - N = total number of consecutive state triplets across all trajectories
    - 3 = three consecutive states (t, t+1, t+2)
...
def extract_state_triplets(train_vals):
    """
    Extract all consecutive state triplets from a single trajectory.
    
    Args:
        train_vals: Full trajectory array (T x 32)
    
    Returns:
        np.ndarray: Array of state triplets, shape (T-2, 3, 4)
    """
    n_timesteps = train_vals.shape[0]
    triplets = []
    
    # Need at least 3 timesteps for a triplet
    for t in range(n_timesteps - 2):
        state1 = extract_state_vector(train_vals, t)
        state2 = extract_state_vector(train_vals, t + 1)
        state3 = extract_state_vector(train_vals, t + 2)
        triplets.append(np.stack([state1, state2, state3]))
...
def filter_trajectory(train_vals, min_length=50, safety_check=True):
    ...
    Args:
        ...
        min_length: Minimum trajectory length (need at least 3 for triplets)
        safety_check: Whether to check safety flags (field 4)
    
    Returns:
        bool: True if trajectory should be kept
    """
    # Check minimum length (need at least 3 timesteps for triplets)
    if train_vals.shape[0] < max(min_length, 3):
        return False
...
def process_all_trajectories(data_dir, min_length=50, max_trajectories=None, 
                            safety_check=True, demo_list=None):
    """
    Process all trajectory files and collect state triplets.
    ...
    Returns:
        tuple: (dataset, stats)
            - dataset: np.ndarray of shape (N, 3, 4)
    ...
    for file_path in tqdm(trajectory_files, desc="Processing trajectories"):
        try:
            ...
            # Extract state triplets
            triplets = extract_state_triplets(train_vals)
            all_triplets.append(triplets)
            
            stats['valid_trajectories'] += 1
            stats['total_triplets'] += triplets.shape[0]
    ...
    dataset = np.concatenate(all_triplets, axis=0)
...
def print_dataset_statistics(dataset, stats):
    ...
    print(f"\nDataset size:")
    print(f"  Total state triplets: {dataset.shape[0]:,}")
...
def validate_dataset(dataset):
    ...
    assert dataset.shape[1] == 3, f"Expected 3 states per triplet, got {dataset.shape[1]}"
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Prepare AMP dataset with 3 consecutive states from real robot trajectories',
    ...
    parser.add_argument(
        '--output-path',
        type=str,
        default='scripts/smooth_policy/amp_triplet_data/amp_triplet_dataset.pt',
...
def main():
    """Main function to prepare AMP triplet dataset."""
    args = parse_args()
    
    print("="*80)
    print("AMP TRIPLET DATASET PREPARATION")
    print("="*80)
    print(f"\nConfiguration:")
    ...
    print("  state_triplets = data['state_triplets']  # Shape: (N, 3, 4)")
    print("  state1 = state_triplets[:, 0, :]  # (N, 4)")
    print("  state2 = state_triplets[:, 1, :]  # (N, 4)")
    print("  state3 = state_triplets[:, 2, :]  # (N, 4)")
