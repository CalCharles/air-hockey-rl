#!/usr/bin/env python3
"""
Fine 3D grid search around the best configs from the coarse search.
Non-overlapping with the coarse grid points.
"""
...
def replay_errors_only(episode_path: str, sim_cfg: dict) -> dict[str, float]:
    """Replay with puck parked at far bottom so it never touches the paddle."""
    ...
    # Park puck at far bottom of table with zero velocity
    state0[4:6] = [-0.9, 0.0]
    state0[6:8] = [0.0, 0.0]
...
def run_3d_grid_search(kp_values, kd_values, density_values):
    ...
    n_combos = len(combos)
    print(f"Fine grid: {len(kp_values)} kp × {len(kd_values)} kd × {len(density_values)} density = {n_combos} combos")
...
def plot_heatmaps_per_density(results, kp_values, kd_values, density_values, out_dir, metric, label):
    ...
    fig.suptitle(f"Fine Grid — Mean {label.title()} Error by density", fontsize=13)
    fig.tight_layout()
...
def plot_density_vs_kp(results, kp_values, kd_values, density_values, out_dir, metric, label):
    ...
    fig.colorbar(im, ax=axes.ravel().tolist(), label="Error (m)", shrink=0.8)
    fig.suptitle(f"Fine Grid — Mean {label.title()} Error: density vs kp", fontsize=13)
    fig.tight_layout()
...
def main():
    kp_values = [5500, 6500, 7500, 8500, 9000]
    kd_values = [100, 300, 400]
    density_values = [1750, 2250, 2750, 3250]

    results = run_3d_grid_search(kp_values, kd_values, density_values)
    ranked = print_leaderboard(results, top_n=20)

    out_dir = SYSID_DIR / "grid_search_results_3d_fine_no_puck"
    ...
    with open(out_dir / "grid_search_3d_fine_no_puck_results.json", "w") as f:
        json.dump(summary, f, indent=2)
    print(f"\nResults saved to: {out_dir / 'grid_search_3d_fine_no_puck_results.json'}")
