...
if __name__ == "__main__":
    ...
    config["air_hockey"]["include_inherited_reward"] = args.include_inherited_reward
    ...
    # ===== torch.profiler instrumentation (profiling-only copy) =====
    import sys
    from torch.profiler import profile, ProfilerActivity, schedule, tensorboard_trace_handler
    _prof = profile(
        activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
        schedule=schedule(wait=2, warmup=2, active=1, repeat=1),
        on_trace_ready=tensorboard_trace_handler(f"profiling/torch_profiler/traces/{args.run_name}"),
        record_shapes=False,
        profile_memory=False,
        with_stack=False,
    )
    _prof.start()
    _PROFILE_STOP_AFTER = 5  # 2 wait + 2 warmup + 1 active = 5

    for iteration in range(1, args.num_iterations + 1):
        _prof.step()
        if iteration > _PROFILE_STOP_AFTER:
            _prof.stop()
            print("[torch.profiler] traces written to profiling/torch_profiler/traces — exiting")
            try:
                envs.close()
            except Exception:
                pass
