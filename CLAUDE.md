...
└── real_configs/  — real-robot rollout / mouse-teleop configs (`tasks/` = per-task eval configs for touch / reach / reach_vel / puck_vel)
...
| **Real-world fixed-policy eval (all five canonical tasks)** | `scripts/td3/extras/async_td3_real_eval.py` (frozen actor, no learner / replay / checkpointing — emits `eval_summary.json` + `eval_per_episode.jsonl`). Two pluggability layers: `--agent {td3,sgcrl,…}` selects how the actor is built (dispatch in `scripts/td3/helper/real_eval_agents.py`); the env config's `task:` selects a `TaskEvalHooks` from `scripts/td3/helper/real_task_eval_hooks.py`, which supplies **both the between-episode reset strategy and the metrics**: juggle / `puck_touch` / `puck_velocity` → `ResetPolicyFSM` puck sweep; `paddle_reach_position(_velocity)` → `PaddleRepositionFSM` (`scripts/td3/helper/real_paddle_reposition_fsm.py`: drive paddle to a random start pose, settle, resample goal — no puck, no reset policy); unknown tasks → `GenericEvalHooks` (puck FSM, bare metrics). Real configs per task: `configs/real_configs/tasks/rollout_td3_{touch,reach,reach_vel,puck_vel}_hist{2,4}.yaml` (juggle: `configs/real_configs/rollout_td3_config_hist{2,4}.yaml`). Full reference: [`notes/docs/training/real-world-eval-pipeline.md`](notes/docs/training/real-world-eval-pipeline.md). |
...
> **hist4 adoption is NOT validated — it measured worse.** The 2026-09-04 like-for-like head-to-head ([`2026-09-04_22-19_hist4-smoothing-five-tasks.md`](notes/scratch/experiments/2026-09-04_22-19_hist4-smoothing-five-tasks.md), runs `runs/td3/tasks_hist4_20260904/`) found hist4 loses on every task with reward headroom, in both sysid and DR: juggle DR back-half **94.5 vs 116.8**, puck_vel DR **25.0 vs 57.7** (max 31 vs 80), juggle sysid **64.3 vs 83.4**; under DR it also delays reach saturation from 200k to ~1.8M steps. touch / reach_vel / reach-sysid sit at their reward ceiling and are indifferent. Single seed per cell, but consistent across all four independent comparisons and across the whole trajectory. Smoothing and the paddle-gain refit moved together, so the cause is not yet isolated. **Revert to `hist_len: 2` (+ kp 9000 / density 3000) unless hist4 is required for real-robot transfer.**
 Run them all with `scripts/td3/run_experiments.py --mode auto --configs configs/td3/tasks/*.yaml --gpus ...`. The reward scales are part of the task definitions (reward classes), not config knobs — see `notes/scratch/experiments/2026-09-04_01-05_sparse-task-collapse-diagnosis.md` for why the sparse tasks need ×10. Other tasks (`puck_juggle`, `puck_strike`, `puck_score`, `puck_goal_position*`, `move_block`, `strike_crowd`, etc.) remain registered in `airhockey/__init__.py` and are callable from Python, but no config files target them.
...
- **Default Box2D config** for one-off scripts: `configs/new_juggle/sysid_best_params_hist2.yaml` (use `sysid_best_params_hist4.yaml` to match the canonical 4-timestep smoothing of the five task configs).
