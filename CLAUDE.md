...
| **Residual RL recipe (sim2sim/sim2real fine-tune)** | Small-gap (<10% zs drop): `…/td3/sim2sim/td3_sim2sim_residual.yaml` (`recency_top50`). Big-gap (>20% zs drop, paddle50) — both 5-seed verified, pick by deployment style: `…/td3/sim2sim/paddle50/td3_residual_v27_ensemble5.yaml` (peak deployment, 1M-verified) or `…/paddle50/td3_residual_v30_explore_lite.yaml` (fire-and-forget; ~4× tighter cross-seed last5 std). Real-world: `…/td3_real_world/td3_residual.yaml` via `async_td3_real_modular.py`. **Read [`notes/docs/training/residual-rl-recipe.md`](notes/docs/training/residual-rl-recipe.md) before running.** |
