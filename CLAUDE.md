...
- **Real-world clamping coverage gap**: [`environments/real-world/paddle-clamping-coverage-gap.md`](notes/docs/environments/real-world/paddle-clamping-coverage-gap.md) — sync-only `apply_negative_z_force` + UR's ~2 s `forceMode` timeout means the paddle goes compliant-free during idle phases. Read before debugging any "robot stops clamping mid-reset" report. **The reset-internal half is now closed by a default-on closed-loop seat check** (`reset_verify_seated`, `AirHockeyReal._verify_paddle_seated`): `reset()` holds the clamp on and polls actual TCP z until it settles (1 cm band / 1 s) or a timeout+retry gives up loudly, so an episode no longer starts with the paddle still descending. The between-episodes gap is still open.
