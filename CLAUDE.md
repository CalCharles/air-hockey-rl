...
| **Recommended TD3 default** | `…/td3/td3_recommended_top50_hist2.yaml` (2-layer, q=25/a=6, hist_len=2 via `sysid_best_params_hist2.yaml`, success_top_fraction=0.5, no bootstrap forcing, no external warmstart — see [depth/update ablations](notes/docs/training/td3-ablations-updates-and-depth.md) and [exploration ablations](notes/docs/training/td3-exploration-ablations.md)). Older hist4 variant (`legacy/td3_recommended.yaml` + `new_juggle/legacy/sysid_best_params_hist*.yaml`) is preserved for reproducibility but no longer the default. |
