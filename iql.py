...
class TrainConfig:
    ...
    max_timesteps: int = int(20000)  # Max time steps to run environment
...
@torch.no_grad()
def eval_actor(
    env: gym.Env, actor: nn.Module, dir, device: str, n_episodes: int, seed: int, renderer: AirHockeyRenderer = None, best_rew_so_far=-100000
) -> np.ndarray:
    # env.seed(seed)
    actor.eval()
    episode_rewards = []
...
def train(config: TrainConfig):
    ...
    dataset = {}
    dataset["observations"] = np_dataset[:,:state_dim]
    dataset["actions"] = np_dataset[:,state_dim:state_dim+action_dim]
    dataset["rewards"] = np_dataset[:,state_dim+action_dim]
    dataset["next_observations"] = np_dataset[:,state_dim+action_dim + 1:state_dim+action_dim + 1 + state_dim]
    dataset["terminals"] = np_dataset[:,state_dim+action_dim + 1 + state_dim]*0
    
    return_reward_range(dataset, 200)
    ...
    for t in range(int(config.max_timesteps)):
        ...
        if (t + 1) % config.eval_freq == 0:
            ...
            eval_scores = eval_actor(
                ...
                seed=config.seed,
                renderer=renderer,
                best_rew_so_far=best_rew,
            )
            print(eval_scores)
            eval_score = eval_scores.mean()
            best_rew = max(best_rew, eval_score)
